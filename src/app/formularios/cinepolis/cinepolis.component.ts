...
export class CinepolisComponent {
  ...
  procesarFormulario() {
    try {
      ...
      if (e instanceof Error) {
        this.error = e.message;  // Accede a 'message' si 'e' es una instancia de 'Error'
      } else {
        this.error = 'Ha ocurrido un error inesperado';  // Mensaje por defecto si no es un objeto de tipo 'Error'
      }
  ...
  calcularTotal(cantidad_boletas: number, cantidad_compradores: number, tarjeta_cineco: string) {
    const precio_por_boleta = 12.000;
